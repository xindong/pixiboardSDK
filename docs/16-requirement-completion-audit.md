...
| Public release gate (`npm pack` / external consumer) | **achieved（本次真实命令证据）** | 在本审计 worktree 执行 `pnpm build:release` 后，`pnpm release:check` 输出 `Packed pixiboardjs@0.1.0-alpha.0: 18 files`、`consumer node imports passed`、`consumer npm run build passed` 和 `Release gate passed: packed manifest and external Node/Vite consumers are valid`；三 public package artifacts 均由 release build 生成。dist/tarball 为临时产物，不入库。 | 每个 release candidate 仍需在目标 commit 重新运行 build + release gate；本状态不意味着 dist 已提交。 |
| SemVer / Changesets / API report / bundle budget | **achieved（本次真实命令证据）** | `.changeset/config.json`、`.changeset/p6-publishability.md`、三个 public package 的 `CHANGELOG.md`、`api-extractor.json`、committed `etc/*.api.md` 和 `bundle-budget.json` 均存在；同一 release run 中 `pnpm api:check` 输出 “API report compare passed”，`pnpm bundle:check` 输出 “Bundle budget passed”。 | 每次 public API 或 bundle budget 改动后重新运行 API/bundle checks，并保持 reports/budgets 同步。 |
...
| `pnpm release:check` | pass：`Packed pixiboardjs@0.1.0-alpha.0: 18 files`、`consumer node imports passed`、`consumer npm run build passed`、`Release gate passed: packed manifest and external Node/Vite consumers are valid` | release gate 使用本地临时 dist/tarball；不把生成产物纳入 Git。 |
...
本审计明确区分“实现/契约存在”和“真实环境 gate 已通过”。当前可以确认的是新 Document 边界、Core/Capabilities/Agent、MCP real deployment、renderer 增量恢复、Plugin API v3 contract、Browser adapter、Desktop scoped fixture、candidate-bound Pixi/Konva evidence runner 以及本次 public release/API/bundle gate 已通过；仍不能提前标记完成的是浏览器数值回归阈值、完整 nightly artifact、真实 media-heavy decode/playback、GPU/heap 指标和 Windows 成功 smoke。旧插件不迁移、不兼容、不重新打包，也不作为任何 SDK parity 证据。
