...
`docs/14-parallel-execution.md` 的 commit/session 汇总只说明工作线声称合并；它不能替代本矩阵要求的测试输出、fixture、发布物或性能数据。尤其是 Document 格式拒绝、MCP、真实 Tauri smoke、API report/Changesets/bundle budget、Konva 和 soak，在这些产物出现前均不得标记 achieved。旧数据 migration/round-trip 不属于完成条件。
