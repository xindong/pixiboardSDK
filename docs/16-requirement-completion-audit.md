...
本审计明确区分“实现/契约存在”和“真实环境 gate 已通过”。当前可以确认的是新 Document 边界、Core/Capabilities/Agent、MCP real deployment、renderer 增量恢复、Plugin API v3 contract、Browser adapter、Desktop scoped fixture、canonical Pixi/Konva 对照以及本次 public release/API/bundle gate 已通过；仍不能提前标记完成的是完整 nightly artifact、真实 media-heavy decode/playback、GPU/heap 指标和 Windows 成功 smoke。旧插件不迁移、不兼容、不重新打包，也不作为任何 SDK parity 证据。
