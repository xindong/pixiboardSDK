...
| Core ↔ Capabilities ↔ Agent equivalence | **achieved** | `packages/capabilities/src/contract.test.ts` 覆盖单 transaction、no-op、abort、headless availability 和错误；`packages/agent-tools/src/contract.test.ts` 对 direct Core、Capabilities、Agent 的 document/revision/ChangeSet/undo/redo、requestId 和 source asset+node 单提交做等价断言。 | 不能复制第二套写入链。transport 已移出 SDK 范围，由接入方自行组装，因此不再有 transport 层部署证据要求。 |
...
|---|---|---|
| `pnpm audit:requirements` | pass：输出本矩阵的静态 evidence/status（含 renderer 19 tests、Konva candidate-bound evidence-only） | 只检查可定位源码、测试、配置和产物；不会把 roadmap 行当完成。 |
...
| `pnpm packages:check` | pass：workspace/public exports/fixtures/benchmark gates | 这是 package 结构门，不等价于 npm tarball 成功。 |
| `git diff --check` | pass | 审计文档和脚本无 whitespace error。 |

## Audit rule

本审计明确区分“实现/契约存在”和“真实环境 gate 已通过”。当前可以确认的是新 Document 边界、Core/Capabilities/Agent、renderer 增量恢复、Plugin API v3 contract、Browser adapter、Desktop scoped fixture、PR Node baseline comparison、candidate-bound Pixi/Konva evidence runner 以及本次 public release/API/bundle gate 已通过；仍不能提前标记完成的是 browser 数值预算、完整 nightly/RC artifact、真实 media-heavy decode/playback、GPU memory/draw calls、硬件 GPU throughput 和 Windows 成功 smoke。旧插件不迁移、不兼容、不重新打包，也不作为任何 SDK parity 证据。
