...
export function createBoardCapabilities(core: BoardCore, services: BoardCapabilityServices = {}): BoardCapabilities {
  ...
  const capabilities: BoardCapabilities = {
    ...
    nodes: {
      ...
      create: async (input, options = {}) => { try { if (!input.nodes.length) throw new CapabilityError("INVALID_INPUT", "nodes must not be empty"); const tx = transact(options.label ?? "Create nodes", options, () => { const nodes: BoardNode[] = []; for (const raw of input.nodes) { aborted(options.signal); const { asset, ...nodeInput } = raw; const refs = asset ? { ...(nodeInput.assetRefs ?? {}), primary: { assetId: asset.id, variant: "original" as const } } : nodeInput.assetRefs; if (asset) core.assets.upsert(asset); nodes.push(core.nodes.create({ rotation: 0, zIndex: 0, ...nodeInput, x: nodeInput.x ?? 0, y: nodeInput.y ?? 0, width: nodeInput.width ?? 100, height: nodeInput.height ?? 100, ...(refs ? { assetRefs: refs } : {}) } as never) as BoardNode); } return nodes; }); return writeResult({ ...tx, nodes: tx.result, requestId: options.requestId }); } catch (error) { throw mapCoreError(error); } },
