...
export type PluginManifest = { id: string; name: string; version: string; apiVersion: typeof PLUGIN_API_VERSION; permissions: readonly PluginPermission[]; contributions?: { panels?: readonly string[]; tools?: readonly string[]; processes?: readonly string[] } };
export type PluginEvent = { type: "change"; revision: number; changeSet: unknown } | { type: "selection:change"; nodeIds: readonly string[]; previousNodeIds: readonly string[] } | { type: "viewport:change"; viewport: unknown; previousViewport: unknown };
export type PluginEventSource = { on(event: "change" | "selection:change" | "viewport:change", listener: (event: PluginEvent) => void): () => void };
export type PluginCanvas = { read(input?: unknown, options?: unknown): Promise<{ nodes: readonly unknown[]; page: { hasMore: boolean; nextCursor?: string }; revision: number; requestId?: string }>; create(input: unknown, options?: unknown): Promise<unknown>; update(input: unknown, options?: unknown): Promise<unknown>; delete(input: unknown, options?: unknown): Promise<unknown> };
